...
const handleRemoveAllImages = async () => {
  ...
  setRemoving(true);
  setRemoveButtonText('Removing'); // Set button text to "Removing..."

  try {
    // Remove all uploaded images from Firebase Storage
    await Promise.all(
      uploadedImages.map(async (image) => {
        const imageRef = ref(storage, `sharedImages/${currentUser.uid}/${selectedContact.clientId}/${image.fileName}`);
        try {
    ...
    // Remove image metadata from Firestore
    ...
    // Remove sharing documents from Firestore
    const photographerSharingRef = doc(db, `users/${currentUser.uid}/sharing/${selectedContact.clientId}`);
    const clientSharingRef = doc(db, `users/${selectedContact.clientId}/sharing/${currentUser.uid}`);

    await Promise.all([
      deleteDoc(photographerSharingRef),
      deleteDoc(clientSharingRef),
    ]);
    ...
    setRemoveButtonText('Remove All Images'); // Reset button text
  }
};
